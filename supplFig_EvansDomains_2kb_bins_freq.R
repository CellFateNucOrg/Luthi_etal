...
p1<-ggplot(df,aes(x=binnedDistance1,y=domainFrequency/1000,fill=domain)) +
  ...
  ylab("Bin domain frequency (x1000)")
p1<-p1 + geom_bar(data=boxdf,aes(x=binnedDistance1, y=domainFrequency/1000,fill=NULL),stat="identity",
...
p2<-ggplot(genomeWide,aes(x=1,y=domainFreq/1000,fill=name)) +
  geom_bar(position="stack",stat="identity")  +
  scale_fill_manual(values=domainClrs) +
  xlab("x") + ylab("Genome-wide domain frequency (x1000)") + ggtitle("")
p2
