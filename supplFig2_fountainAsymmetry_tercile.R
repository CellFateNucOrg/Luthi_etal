...
ignoreBins<-c(">-10","-0",">10","Asym.Q2")

selected=!df$binnedDistance1 %in% ignoreBins & !(df$AsymmetryTercile %in% ignoreBins) & df$log2TPMtev >0
countLabels_a<-df[selected,]%>%
  dplyr::group_by(binnedDistance1,AsymmetryTercile,.drop=T) %>%
  dplyr::summarise(count=dplyr::n())

# calculate stats
stat_df<-df[!(df$AsymmetryTercile %in% ignoreBins) & df$log2TPMtev >0,] %>%
  rstatix::group_by(AsymmetryTercile,binnedDistance,drop=T) %>% filter(binnedDistance==2) %>%
  rstatix::t_test(log2TPMtev~binnedDistance1)  %>% filter(group2 %in% 2:10) %>%
...
selected=!df$binnedDistance1 %in% ignoreBins & !(df$AsymmetryTercile %in% ignoreBins) & df$log2TPMcoh1 >0
countLabels_a<-df[selected,]%>%
  dplyr::group_by(binnedDistance1,AsymmetryTercile,.drop=T) %>%
  dplyr::summarise(count=dplyr::n())

# calculate stats
stat_df<-df[!(df$AsymmetryTercile %in% ignoreBins) & df$log2TPMcoh1 >0,] %>%
  rstatix::group_by(AsymmetryTercile,binnedDistance,drop=T) %>% filter(binnedDistance==2) %>%
  rstatix::t_test(log2TPMcoh1~binnedDistance1)  %>% filter(group2 %in% 2:10) %>%
  adjust_pvalue(method="BH") %>%
  add_significance("p.adj") %>%
  filter(!group1 %in% ignoreBins) %>%
  add_xy_position(x="binnedDistance1") %>%
...
ggsave(filename=paste0(finalFigDir,"/supplFig2_fountainAsymmetry_tercile.pdf"),
