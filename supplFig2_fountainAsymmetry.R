...
selected=!df$binnedDistance1 %in% ignoreBins & !(df$AsymmetryQuintile %in% ignoreBins) & df$log2TPMtev >0
...
selected=!df$binnedDistance1 %in% ignoreBins & !(df$AsymmetryQuintile %in% ignoreBins) & df$log2TPMcoh1 >0
countLabels_a<-df[selected,]%>%
  dplyr::group_by(binnedDistance1,AsymmetryQuintile,.drop=T) %>%
  dplyr::summarise(count=dplyr::n())
...
boxdf<-df %>% filter(XvA=="A",AsymmetryQuintile %in% c("Asym.Q1","Asym.Q5")) %>%
  ...
  filter(binnedDistance1=="0") %>% select(-AsymmetryQuintile,-state,-fountainLocation) %>% distinct()
