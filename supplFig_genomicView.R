...
for(i in which(fountains$ScoreTercile=="3")){
  ...
  for(region in regions){
    ...
    pdf(file=paste0(fountainFigDir,"/reg",region/1e3,"kb_res",resolution/1e3,"kb/",
                    ...
                    end(fountains)[i],".pdf"),width=18,height=25,paper="a4")
    ...
    currentHeight=0
    pageCreate(width = 18, height = 23, params=params, showGuides=FALSE)
    ...
    annoGenomeLabel(
      plot = hicPlot,
      params=params,
      y = currentHeight+hicHeight, height = textHeight,
    )
    currentHeight<-currentHeight+ hicHeight + textHeight + 0.2
    ...
    p<-plotRanges(chromHMM,
                  params=params,
                  y = currentHeight+textHeight, height=rangesHeight,
                  boxHeight=boxHeight,fill=chromHMM$itemRgb)
    currentHeight<-currentHeight + textHeight+rangesHeight + 0.1
    ...
    p<-plotSignal(coh1lfc,
                  ...
                  linecolor="red2",
                  negData=T)
    plotText(label = "COH-1 cleavage RNA log2FC", fontcolor = "black",
             y = currentHeight, params=params)
    currentHeight<-currentHeight + signalHeight + 0.1
    ...
    p<-plotSignal(tpm366,
                  ...
                  linecolor="blue4",
                  negData=T)
    plotText(label = "TEV-only log2(TPM)", fontcolor = "blue4",
             y = currentHeight, params=params)
    currentHeight<-currentHeight + signalHeight + 0.1
    ...
    # p<-plotTranscripts(params=params,
    ...
    #pageGuideHide()
    plotText(label= paste0(fountains$name[i],"_",seqnames(fountains)[i],"_",
                           start(fountains)[i],"-",end(fountains)[i],"_score",
                           fountains$ScoreQuintile[i],"_asymmetry",
                           fountains$AsymmetryQuintile[i]),
             fontcolor = "black",
             y = currentHeight+0.2, params=params)
    dev.off()
